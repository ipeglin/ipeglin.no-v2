  ...
  onBeforeMount(async () => {
    try {
      ...
      const data = await axios.get(
        'https://api.github.com/users/ipeglin/repos'
      );
    ...
    try {
      ...
      const data = await axios.get(
        'https://api.github.com/users/ipeglin/followers'
      );
    ...
    if (Object.keys(repoImages.value).length !== 0)
      return console.info('Images have allready been fetched');
        ...
        res.items.forEach((itemRef) => {
          ...
          if (repoNames.includes(imageName)) {
            getDownloadURL(itemRef).then(
              (url) => (repoImages.value[imageName] = url)
            );
          }
        });
      })
